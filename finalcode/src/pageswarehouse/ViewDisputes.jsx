...
import {getdisputeswarehouse} from "../actions/warehouse"
...
const ViewDisputes = () => {
 ...
 const { id } = useParams("");
  ...
  const getiddata = async () => {
    ...
    const data=await dispatch(getdisputeswarehouse(tosend));
    console.log(data);
  ...
  const handleViewWarehouse=(id)=>{
    console.log(id)
    window.location.href=`/ViewDisputeWarehouse/warehouse/${id}`
                                  ...
                                  {displaywarehose.length===0 ? (
                                   ...
                                   displaywarehose.map((item,index)=>(
                                      <MDBTableBody key={index}>
                                        ...
                                        <tr>
                                          <td scope='row' className='selector' onClick={()=>handleViewWarehouse(item._id)}> {item.warehouseemail}</td>
