...
import {links2,links2ware} from "../data/dummy";
          ...
          <>
            <div className='flex justify-between items-center'>
              ...
              font-extrabold trackig-tight dark:text-white text-slate-900"> 
                ...
                <SiShopware/> <span>Stock Logistics</span>
              </Link>
            ...
            {authdata.accountstanding=="deactivated" || authdata.accountstanding=="inactive"? ( <div className='mt-10'> 
                  {links2ware.map((item)=>(
                    <div key={item.title}>
                        ...
                        {item.links.map((link)=>(
                          <NavLink to={`/${link.link+email}`} 
                          key={link.name} onClick={handleCloseSideBar}
                          style={({isActive})=>({
                            backgroundColor:isActive ? '#961818' :''
                          })}
                          className={({isActive})=>
